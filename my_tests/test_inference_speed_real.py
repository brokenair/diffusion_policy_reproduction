...
def load_dataset(zarr_path, image_key='img_128x128'):
    """
    加载训练数据集。
    """
...
def test_inference_speed(policy, cfg, device, replay_buffer, image_key, 
    ...
    if len(inference_times) > 0:
        ...
        # 计算action的典型范围（用于评估误差的相对大小）
        all_true_actions_array = results['all_true_actions']
...
def main():
    ...
    # 加载数据集
    print("\n加载数据集...")
    replay_buffer, image_key = load_dataset(ZARR_PATH, image_key='img_128x128')
