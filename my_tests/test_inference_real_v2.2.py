    ...
    try:
        ...
        while True:
            ...
            if len(action_queue) > 0:
                ...
                current_ee_pose = controller.get_end_effector_pose()
                target_xy = current_ee_pose.translation[:2]
            
            # 读取当前图像用于显示（如果不在推理中）
            if len(action_queue) > 0 or latest_actions_clipped is None:
                frames = pipeline.wait_for_frames()
                color_frame = frames.get_color_frame()
                if color_frame:
                    frame_bgr = np.asanyarray(color_frame.get_data())
                    frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
                    frame_display = process_image(frame_rgb, crop_x1, crop_y1, crop_x2, crop_y2, DISPLAY_TARGET_PIXELS)
                else:
                    continue
            ...
            # 当前位置（蓝色点）
            current_ee_pose = controller.get_end_effector_pose()
            current_pos_xy = current_ee_pose.translation[:2]
